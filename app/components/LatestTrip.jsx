...
const LatestTrip = async () => {
  ...
  const upcomingBooking = await getUpcomingBookings(userId);

  let currentBooking;

  if (upcomingBooking.length === 0) {
    currentBooking = await getCurrentBookings(userId);
  }
  return (
    <div className="flex flex-col gap-8 pl-8 pr-12 py-12">
      {upcomingBooking.length > 0 ? (
        <p>{upcomingBooking.at(-1).endDate}</p>
      ) : currentBooking.length > 0 ? (
        <p>{currentBooking.at(-1).endDate}</p>
      ) : (
        <>
          <svg
            viewBox="0 0 48 48"
